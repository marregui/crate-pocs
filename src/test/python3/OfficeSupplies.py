...
if __name__ == "__main__":
    print("date, region, rep, item, units, unit_price")
    for line in RAW_DATA.split('\n'):
        line = line.strip()
        if line:
            date, region, rep, item, units, unit_price = line.split(',')
            dd, Mon, yyyy = date.split("-")
            mm = int(MONTHS.index(Mon) + 1)
            date = '{}-{:02d}-{:02d}'.format(yyyy, mm, int(dd))
            ts = int(time.mktime(time.strptime(date, '%Y-%m-%d')))
            print("{}, '{}', '{}', '{}', {}, {}".format(ts, region, rep, item, units, unit_price))
